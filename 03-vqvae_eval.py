...
checkpoint = torch.load("checkpoint/checkpoint_vqvae_randomwindow/last_model.pth", map_location='cpu')
...
def extract_embeddings(dataloader):
    ...
    for idx, (x, y) in enumerate(dataloader):
        ...
        with torch.no_grad():
            tokens = model.forward_token(x)
            x_stack.extend(tokens.detach())
            y_stack.extend(y)
